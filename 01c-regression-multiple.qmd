...
model3 <- lm(I(log(CWD.BASA)) ~ RIP.DENS + I(RIP.DENS^2), data = CWD)
summary(model)
confint(model)
...
```{r}
#| echo: true
model_l <- lm(length ~ phosphor, data = needles)
...
model_l2 <- lm(length ~ nitrogen + phosphor
               + potassium + residu, data = needles)
summary(model_l2)
```
```{r echo = FALSE}
model_l2 <- lm(length ~ nitrogen + phosphor + potassium + residu, data = needles)
...
#| echo: false
par(mfrow = c(2,2))
plot(needles$nitrogen, resid(model_l2))
lines(lowess(needles$nitrogen, resid(model_l2)))
plot(needles$phosphor, resid(model_l2))
lines(lowess(needles$phosphor, resid(model_l2)))
plot(needles$potassium, resid(model_l2))
lines(lowess(needles$potassium, resid(model_l2)))
plot(needles$residu, resid(model_l2))
lines(lowess(needles$residu, resid(model_l2)))
```
...
```{r}
#| echo: true
model_l3 <- lm(length ~ nitrogen * phosphor, data = needles)
summary(model_l3)
```
\normalsize
...
```{r}
#| echo: true
model_l4 <- lm(length ~ nitrogen + phosphor, data = needles)
summary(model_l4)
```
\normalsize
...
- This is **not** done in exhaustive way:
  ...
  - Because multiple testing can, by mere coincidence, lead to incorrect conclusions that certain higher-order terms are significant.
...
- Recall: Confounders are factors
