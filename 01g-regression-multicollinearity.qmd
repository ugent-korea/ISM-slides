...
model_l5 <- lm(length ~ nitrogen * phosphor + potassium
               + phosphor * residu, data = needles)
model_l8 <- lm(length ~ nitrogen * phosphor + potassium, data = needles)
...
- **Can possibly cause problems**: if there is strong correlation between 2 predictors $X_1$ and $X_2$, it becomes difficult to discern the effect of $X_1$ from the effect of $X_2$
...
- **Strong correlation** between predictors:
  - Example: usually strong correlation between $X_1$ and $X_1X_2$
...
```{r}
nitrogenphospor <- needles$nitrogen * needles$phosphor
minerals <- cbind(needles$nitrogen, needles$phosphor, needles$potassium, nitrogenphospor)
colnames(minerals)[1:3] <- c("nitrogen", "phosphor", "potassium")
...
```{r}
cnitrogen <- needles$nitrogen - mean(needles$nitrogen)
cphosphor <- needles$phosphor - mean(needles$phosphor)
cpotassium <- needles$potassium - mean(needles$potassium)
cresidu <- needles$residu - mean(needles$residu)
...
- Average quadratic distance between estimated and true coefficients is proportional to the average VIF.
...
library(car)
needles_c <- data.frame(length = needles$length, cnitrogen = cnitrogen,
                        cphosphor = cphosphor, cpotassium = cpotassium,
                        cresidu = cresidu)
model_l8b <- lm(length ~ cnitrogen * cphosphor + cpotassium, data = needles_c)
model_l5b <- lm(length ~ cnitrogen * cphosphor + cpotassium + cphosphor * cresidu, data = needles_c)
v1 <- vif(model_l5)
v2 <- vif(model_l5b)
par(mfrow=c(1,2))
...
model_bf <- lm(bodyfat ~ triceps.skinfold.thickness +
                         thigh.circumference +
                         midarm.circumference, data = body.fat)
summary(model_bf)
```
