...
library(caret)
model5 <- knn3(diagnosis ~ radius_mean + texture_mean, data = train, k = 5)
...
Model: $\texttt{diagnosis} \sim \texttt{radius} + \texttt{texture}$
...
Model: $\texttt{diagnosis} \sim \texttt{radius} + \texttt{texture} + \texttt{radius}:\texttt{texture}$
