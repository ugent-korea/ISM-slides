...
plot(NULL, xlim = c(-4, 10), ylim = c(0, 6), xlab = "", ylab = "")
#lines(xplot, yplot_optim, lty = 1, lw = 2, col = "red")
points(xdata, ydata, pch = 19)
...
- Nonlinear least squares: $A = 5.359$, $k = 1.597$, $x_\text{mid} = 2.450$.
...
$$
- Here $\sigma^2 \approx 0.523/17 = 0.031$ (true value $\sigma^2 = (0.2)^2 = 0.04$)
...
\begin{multline*}
  J(\theta) \approx
    J(\theta_\text{best}) +
    \sum_{i = 1}^p \underbrace{\frac{\partial J}{\partial \theta_i}(\theta_\text{best})}_{= 0}(\theta - \theta_\text{best})_i + \\
    \frac{1}{2} \sum_{i,j=1}^p \frac{\partial^2 J}{\partial \theta_i \partial \theta_j}
...
$$
with $\mathcal{I}$ the $p \times p$ **Fisher Information Matrix (FIM)**:
$$
  \mathcal{I}_{ij} =
...
- Can be used to construct correlations between parameters:
$$
...
- Parameter estimates: $A = 5.359$, $k = 1.597$, $x_\text{mid} = 2.450$.
...
- Correlation between $A$ and $k$: $R = -0.0034/\sqrt{0.0057 \times 0.0169} = -0.346$.
